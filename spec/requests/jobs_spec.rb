...
# initialize test data 
  ...
  let(:user) { FactoryBot.create(:user, email: 'farah@gmail.com', password: 'password') }
  let(:admin_user) { FactoryBot.create(:user, email: 'admin@test.com', password: 'password', admin: true) }
  ...
  describe 'GET /jobs/:id' , autodoc: true do
    ...
    context 'when the record does not exist' do
      ...
      it 'returns a not found message' do
        expect(response.body).to match("{\"message\":\"Couldn't find Job with 'id'=55\"}")
      end
    end
  end
